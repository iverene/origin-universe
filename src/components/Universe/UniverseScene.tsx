...
import { MilkyWayFocus } from "@/components/Universe/MilkyWayFocus";
...
function CinematicPostprocessing({
  ...
  const { gl } = useThree();
  const canUseEffects = gl.getContext().getContextAttributes() !== null;
  ...
  return (
    <EffectComposer multisampling={0} enableNormalPass={false}>
      <Bloom intensity={bloom * (1 - blackout)} luminanceThreshold={0.13} luminanceSmoothing={0.84} mipmapBlur />
...
export function UniverseScene({ progress }: UniverseSceneProps) {
  ...
  const firstLightEra = smoothstep(0.25, 0.34, progress);
  const darkAges = smoothstep(0.32, 0.42, progress) * (1 - smoothstep(0.42, 0.52, progress));
  ...
  const matterBloom = smoothstep(0.18, 0.29, progress) * (1 - smoothstep(0.36, 0.5, progress));
  const starOpacity = firstStars * (1 - farFuture * 0.82);
  const dustOpacity = smoothstep(0.28, 0.44, progress) * 0.86 * (1 - farFuture * 0.45);
  const nebulaOpacity = smoothstep(0.34, 0.56, progress) * (1 - farFuture * 0.76);
  const galaxyOpacity = galaxies * (1 - farFuture * 0.55);
  const asteroidOpacity = solar * (1 - smoothstep(0.9, 0.98, progress));
  const background = farFuture > 0.7 ? "#000000" : darkAges > 0.45 ? "#000003" : firstLight < 0.08 ? "#000000" : "#000006";
  const bloom = bigBangBloom * 1.25 + matterBloom * 0.64 + firstLightEra * 0.36 + firstStars * 0.34 + nebulaOpacity * 0.18;

  return (
    <>
      <color attach="background" args={[background]} />
      <fog attach="fog" args={["#01020a", 120, 1100]} />
      <ambientLight intensity={(firstLight * 0.08 + firstLightEra * 0.08 + firstStars * 0.08) * (1 - farFuture * 0.9)} color="#7284b8" />
      ...
      <BirthOfMatter progress={progress} />
      <FirstLight progress={progress} />
      ...
      <DistantFuture progress={progress} />
      <CinematicPostprocessing blackout={blackout} bloom={bloom} />
